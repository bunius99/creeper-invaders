;(function() {
  ...
  var Game = function() {
    ...
    function init(level) {
      ...
      self.bodies = self.bodies.concat(createInvaders(self));

      self.gameOver = false;
      self.fad=false;

      self.player = new Player(self, self.gameSize);
    
      // Add the player to the bodies array.
      self.bodies = self.bodies.concat(self.player);  
    }
  ...
  Game.prototype = {
    ...
    update: function() {
      ...
      var isInvader = function(b) {
        return b instanceof Invader;
      }
      
      if (!notCollidingWithAnything(self.player)) {
        this.gameOver = true;
      } else if (!this.bodies.some(isInvader)) {
        this.fad = true;
      }
    ...
    draw: function(screen, gameSize) {
      screen.fillStyle = levelColors[this.level]
      screen.fillRect(0, 0, gameSize.x, gameSize.y);    

      if (this.gameOver) {
        screen.font = "90px Comic Sans MS";
  ...
  Invader.prototype = {
    ...
    update: function() {
      ...
      if (this.patrolX < 0 || this.patrolX > this.game.gameSize.x-300) {
    ...
    draw: function(screen) {
      screen.save()
      screen.fillStyle = 'green'
      var upperLeftX = this.center.x - this.size.x / 2
      var upperLeftY = this.center.y - this.size.y / 2
      // green face
      screen.fillRect(
        upperLeftX,
        upperLeftY,
      ...
      screen.fillRect(
        upperLeftX + this.size.x / 8,
        upperLeftY + this.size.y / 8,
        this.size.x / 4,
        this.size.y / 4
      )
      screen.fillRect(
        upperLeftX + 5 * (this.size.x / 8),
        upperLeftY + this.size.y / 8,
        this.size.x / 4,
        this.size.y / 4
      )
      screen.fillRect(
        upperLeftX + 2 * (this.size.x / 8),
        upperLeftY + 5 * (this.size.y / 8),
        this.size.x / 2,
        this.size.y / 4
      )
      screen.restore()
  ...
  Player.prototype = {
    ...
    update: function() {
      ...
      // If S key is down...
      if (this.keyboarder.isDown(this.keyboarder.KEYS.S) && this.bulletTicks == 0) {
        this.bulletTicks = 10;
    ...
    draw: function(screen) {
      ...
      screen.fillRect(
        upperLeftX + this.size.x / 8,
        upperLeftY + 4 * (this.size.y / 8),
        this.size.x / 8,
        this.size.y / 8
      )
      screen.fillRect(
        upperLeftX + 6 * (this.size.x / 8),
        upperLeftY + 4 * this.size.y / 8,
        this.size.x / 8,
        this.size.y / 8
      )
